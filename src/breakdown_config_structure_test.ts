...
Deno.test("Structure: BreakdownConfig API Contract Validation", async (t) => {
  console.log("Testing BreakdownConfig API contracts");
...
Deno.test("Structure: BreakdownConfig Type Constraint Validation", async (t) => {
  ...
  await t.step("Result types should be properly constrained", () => {
    ...
    if (result.success) {
      ...
      assertEquals(result.data instanceof BreakdownConfig, true, "Data should be BreakdownConfig instance");
    } else {
  ...
  await t.step("Base directory validation should be consistent", () => {
    ...
    assertEquals(emptyDirResult.success, true, "Empty base directory should default to current dir");
...
Deno.test("Structure: BreakdownConfig Interface Consistency", async (t) => {
  ...
  await t.step("Safe and Legacy methods should be consistent", async () => {
    ...
    if (safeLoadResult.success) {
      ...
      if (safeWorkingDir.success) {
        assertEquals(safeWorkingDir.data, legacyWorkingDir, "Safe and legacy methods should return same data");
      }
    } else {
      // If config loading fails, just verify method signatures exist
      assertEquals(typeof safeConfig.data.getWorkingDirSafe, "function", "Safe method should exist");
  ...
  await t.step("Configuration access patterns should be uniform", async () => {
    ...
    if (loadResult.success && getResult.success) {
      assertEquals(typeof loadResult.data, typeof getResult.data, "Config data types should be consistent");
    }
...
Deno.test("Structure: BreakdownConfig Error Handling Structure", async (t) => {
  ...
  await t.step("Error types should be structured consistently", () => {
    ...
    if (!errorResult.success) {
      ...
      // Error should follow UnifiedError structure
      assertExists(error.kind, "Error should have kind property");
      assertExists(error.message, "Error should have message property");
  ...
  await t.step("Error propagation should be predictable", async () => {
    ...
    if (!loadResult.success && !getResult.success) {
      assertEquals(typeof loadResult.error.kind, typeof getResult.error.kind, "Error kinds should have same type");
      assertEquals(typeof loadResult.error.message, typeof getResult.error.message, "Error messages should have same type");
    }
