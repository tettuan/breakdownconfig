...
export class ConfigValidator {
  static validateAppConfig(config: unknown): asserts config is AppConfig {
    ...
    if (!working_dir || typeof working_dir !== "string") {
      ErrorManager.throwError(ErrorCode.REQUIRED_FIELD_MISSING, "working_dir is required and must be a string");
    }

    if (!app_prompt || typeof app_prompt !== "object") {
      ErrorManager.throwError(ErrorCode.REQUIRED_FIELD_MISSING, "app_prompt is required and must be an object");
    }

    if (!app_prompt.base_dir || typeof app_prompt.base_dir !== "string") {
      ErrorManager.throwError(ErrorCode.REQUIRED_FIELD_MISSING, "app_prompt.base_dir is required and must be a string");
    }

    if (!app_schema || typeof app_schema !== "object") {
      ErrorManager.throwError(ErrorCode.REQUIRED_FIELD_MISSING, "app_schema is required and must be an object");
    }

    if (!app_schema.base_dir || typeof app_schema.base_dir !== "string") {
      ErrorManager.throwError(ErrorCode.REQUIRED_FIELD_MISSING, "app_schema.base_dir is required and must be a string");
    }
    ...
    if (userConfig.app_prompt) {
      ...
      if (userConfig.app_prompt.base_dir) {
        if (typeof userConfig.app_prompt.base_dir !== "string") {
          ErrorManager.throwError(ErrorCode.INVALID_FIELD_TYPE, "app_prompt.base_dir must be a string");
        }
    ...
    if (userConfig.app_schema) {
      ...
      if (userConfig.app_schema.base_dir) {
        if (typeof userConfig.app_schema.base_dir !== "string") {
          ErrorManager.throwError(ErrorCode.INVALID_FIELD_TYPE, "app_schema.base_dir must be a string");
        }
  ...
  private static validatePath(path: string): void {
    ...
    if (invalidChars.test(path)) {
      ErrorManager.throwError(ErrorCode.INVALID_PATH_FORMAT, `Path contains invalid characters: ${path}`);
    }

    // パストラバーサルのチェック
    if (path.includes("..")) {
      ErrorManager.throwError(ErrorCode.PATH_TRAVERSAL_DETECTED, `Path traversal detected in: ${path}`);
    }

    // 絶対パスのチェック
    if (path.startsWith("/")) {
      ErrorManager.throwError(ErrorCode.ABSOLUTE_PATH_NOT_ALLOWED, `Absolute path not allowed: ${path}`);
    }
  }
} 