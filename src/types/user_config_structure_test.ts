...
Deno.test("Structure: UserConfig Discriminated Union Integrity", async (t) => {
  ...
  await t.step("All union variants should have proper kind discriminators", () => {
    ...
    assertEquals(promptOnly.kind, "prompt-only", "PromptOnlyUserConfig should have kind 'prompt-only'");
    assertEquals(schemaOnly.kind, "schema-only", "SchemaOnlyUserConfig should have kind 'schema-only'");
  ...
  await t.step("Union variants should have correct property structures", () => {
    ...
    assertEquals(Object.keys(empty).sort(), ["customFields", "kind"], "EmptyUserConfig should have kind and customFields properties");
    
    // PromptOnlyUserConfig should have kind, app_prompt, and customFields
    assertEquals(Object.keys(promptOnly).sort(), ["app_prompt", "customFields", "kind"], "PromptOnlyUserConfig should have kind, app_prompt, and customFields");
    assertEquals(promptOnly.app_prompt.base_dir, "/test/prompt", "PromptOnlyUserConfig should store prompt base_dir");
    
    // SchemaOnlyUserConfig should have kind, app_schema, and customFields
    assertEquals(Object.keys(schemaOnly).sort(), ["app_schema", "customFields", "kind"], "SchemaOnlyUserConfig should have kind, app_schema, and customFields");
    assertEquals(schemaOnly.app_schema.base_dir, "/test/schema", "SchemaOnlyUserConfig should store schema base_dir");
    
    // CompleteUserConfig should have kind, app_prompt, app_schema, and customFields
    assertEquals(Object.keys(complete).sort(), ["app_prompt", "app_schema", "customFields", "kind"], "CompleteUserConfig should have all properties");
    assertEquals(complete.app_prompt.base_dir, "/test/prompt", "CompleteUserConfig should store prompt base_dir");
    assertEquals(complete.app_schema.base_dir, "/test/schema", "CompleteUserConfig should store schema base_dir");
  ...
  await t.step("No optional properties should exist in discriminated union", () => {
    ...
    for (const variant of variants) {
      ...
      if (variant.kind === "prompt-only" || variant.kind === "complete") {
        assertExists((variant as any).app_prompt, "Prompt variants should have defined app_prompt");
      }
      
      if (variant.kind === "schema-only" || variant.kind === "complete") {
        assertExists((variant as any).app_schema, "Schema variants should have defined app_schema");
      }
    }
...
Deno.test("Structure: UserConfigFactory Contract Validation", async (t) => {
  ...
  await t.step("Factory methods should produce valid discriminated unions", () => {
    ...
    for (const variant of variants) {
      ...
      assertEquals(validKinds.includes(variant.kind), true, `Kind "${variant.kind}" should be valid`);
    }
...
Deno.test("Structure: UserConfigGuards Type Safety", async (t) => {
  ...
  await t.step("Type guards should correctly identify union variants", () => {
    ...
    assertEquals(UserConfigGuards.isEmpty(promptOnly), false, "isEmpty should reject non-empty config");
    
    // Test isPromptOnly
    assertEquals(UserConfigGuards.isPromptOnly(promptOnly), true, "isPromptOnly should identify prompt-only config");
    assertEquals(UserConfigGuards.isPromptOnly(empty), false, "isPromptOnly should reject non-prompt-only config");
    
    // Test isSchemaOnly
    assertEquals(UserConfigGuards.isSchemaOnly(schemaOnly), true, "isSchemaOnly should identify schema-only config");
    assertEquals(UserConfigGuards.isSchemaOnly(complete), false, "isSchemaOnly should reject non-schema-only config");
    
    // Test isComplete
    assertEquals(UserConfigGuards.isComplete(complete), true, "isComplete should identify complete config");
    assertEquals(UserConfigGuards.isComplete(schemaOnly), false, "isComplete should reject non-complete config");
  ...
  await t.step("Type guards should provide proper type narrowing", () => {
    ...
    for (const config of configs) {
      if (UserConfigGuards.isEmpty(config)) {
        ...
        assertEquals(Object.keys(config).length, 2, "Empty config should have kind and customFields");
      }
