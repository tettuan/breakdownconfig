...
Deno.test("Structure: ConfigProfile Discriminated Union Integrity", async (t) => {
  ...
  await t.step("AppOnlyProfile should have correct structure", () => {
    ...
    assertEquals(appOnly.source.appConfigPath, "/test/app.config", "Should have correct config path");
  ...
  await t.step("MergedProfile should have correct structure", () => {
    ...
    assertEquals(merged.source.appConfigPath, "/test/app.config", "Should have correct app config path");
    assertEquals(merged.source.userConfigPath, "/test/user.config", "Should have correct user config path");
...
Deno.test("Structure: ConfigProfileFactory Contract Validation", async (t) => {
  ...
  await t.step("Factory methods should have correct signatures", () => {
    ...
    const appOnlyResult = ConfigProfileFactory.createAppOnly(mockAppConfig, "test", "/base/app.config", false);
    ...
    const mergedResult = ConfigProfileFactory.createMerged(mockAppConfig, userConfig, "test", "/base/app.config", "/base/user.config");
  ...
  await t.step("Factory should validate input parameters", () => {
    ...
    const validAppOnlyResult = ConfigProfileFactory.createAppOnly(mockAppConfig, "test", "/valid/app.config", true);
    assertEquals(validAppOnlyResult.success, true, "Valid parameters should succeed");
    if (validAppOnlyResult.success) {
      assertEquals(validAppOnlyResult.data.kind, "app-only", "Valid parameters should create AppOnlyProfile");
    }
    
    const userConfig = UserConfigFactory.createPromptOnly("/test/prompt");
    const validMergedResult = ConfigProfileFactory.createMerged(mockAppConfig, userConfig, "test", "/valid/app.config", "/valid/user.config");
    assertEquals(validMergedResult.success, true, "Valid parameters should succeed");
    if (validMergedResult.success) {
      assertEquals(validMergedResult.data.kind, "merged", "Valid parameters should create MergedProfile");
    }
  ...
  await t.step("Factory should produce consistent profile structures", () => {
    ...
    const appOnly1Result = ConfigProfileFactory.createAppOnly(mockAppConfig, "production", "/base1/app.config", false);
    const appOnly2Result = ConfigProfileFactory.createAppOnly(mockAppConfig, "production", "/base2/app.config", false);
    ...
    if (appOnly1Result.success && appOnly2Result.success) {
      ...
      assertEquals(appOnly1.source.appConfigPath !== appOnly2.source.appConfigPath, true, "Config paths should differ when specified differently");
    }
...
Deno.test("Structure: MergedConfig Schema Validation", async (t) => {
  ...
  await t.step("MergedConfig should combine AppConfig and UserConfig properly", () => {
    ...
    const profileResult = ConfigProfileFactory.createMerged(mockAppConfig, userConfig, "test", "/base/app.config", "/base/user.config");
    ...
    if (profileResult.success) {
      ...
      assertEquals(profile.config.working_dir, ".agent/breakdown", "Should preserve AppConfig working_dir");
  ...
  await t.step("Profile directory resolution should be consistent", () => {
    ...
    const profileResult = ConfigProfileFactory.createAppOnly(mockAppConfig, "development", "/override/app.config", true);
    ...
    if (profileResult.success) {
      ...
      assertEquals(profile.source.appConfigPath, "/override/app.config", "Config path should use override when provided");
      assertEquals(profile.profileName, "development", "ProfileName should match requested profile");
      assertEquals(profile.config.working_dir, ".agent/breakdown", "Should preserve AppConfig working_dir");
    }
  ...
  await t.step("Configuration merging should preserve type safety", () => {
    ...
    const profileResults = [
      ConfigProfileFactory.createMerged(mockAppConfig, emptyUser, "strict", "/base/app.config", "/base/user.config"),
      ConfigProfileFactory.createMerged(mockAppConfig, promptUser, "strict", "/base/app.config", "/base/user.config"),
      ConfigProfileFactory.createMerged(mockAppConfig, schemaUser, "strict", "/base/app.config", "/base/user.config"),
      ConfigProfileFactory.createMerged(mockAppConfig, completeUser, "strict", "/base/app.config", "/base/user.config")
    ];
    ...
    for (const profileResult of profileResults) {
      ...
      if (profileResult.success) {
        ...
        assertEquals(profile.source.userConfigExists, true, "All merged profiles should have userConfig");
        assertEquals(profile.config.working_dir, ".agent/breakdown", "AppConfig should be preserved");
      }
    }
...
Deno.test("Structure: Legacy Compatibility Structure", async (t) => {
  ...
  await t.step("ConfigProfile should maintain backward compatibility interfaces", () => {
    ...
    const profileResult = ConfigProfileFactory.createAppOnly(mockAppConfig, "legacy", "/base/app.config", false);
    ...
    if (profileResult.success) {
      ...
      assertEquals(profile.source.appConfigPath, "/base/app.config", "Should have config path for legacy access");
  ...
  await t.step("Profile evolution should maintain structure integrity", () => {
    ...
    const profileResult = ConfigProfileFactory.createAppOnly(mockAppConfig, "v1", "/current/app.config", true);
    ...
    if (profileResult.success) {
      ...
      assertEquals(typeof profile.source.appConfigPath, "string", "AppConfigPath should remain string");
    }
