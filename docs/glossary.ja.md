...
- **場所**: `{ベースディレクトリ}/.agent/climpt/config/app.yml` または `{ベースディレクトリ}/.agent/climpt/config/{プロファイルプレフィックス}-app.yml`
...
- **場所**: `{working_dir}/.agent/climpt/config/user.yml` または `{working_dir}/.agent/climpt/config/{プロファイルプレフィックス}-user.yml`
...
- **用途**: ユーザー設定ファイル（`{working_dir}/.agent/climpt/config/user.yml`等）の配置場所を決定
- **デフォルト値**: "./.agent/climpt"
...
- **例**: `app_prompt.base_dir: "./.agent/climpt/prompts/app"`
...
- **定義**: ライブラリ専用のログ出力システム (`@tettuan/breakdownlogger@^1.1.3`)
...
- **デフォルト値**: "./.agent/climpt/prompts/app"
- **用途**: プロンプトファイルの場所を指定
...
- **定義**: Schemaファイルの設定項目
- **設定項目**: `base_dir` (Baseフォルダの指定)
- **デフォルト値**: "./.agent/climpt/schema/app"
- **用途**: Schemaファイルの場所を指定
...
- **アプリケーション設定ファイル**: `ベースディレクトリ + /.agent/climpt/config/ + [プロファイルプレフィックス-]app.yml`
- **ユーザー設定ファイル**: `working_dir + /.agent/climpt/config/ + [プロファイルプレフィックス-]user.yml`

### 設定例での比較

```yaml
# アプリケーション設定ファイル（app.yml）
working_dir: "./.agent/climpt" # システム全体の基準
app_prompt:
  base_dir: "./.agent/climpt/prompts/app" # プロンプト機能の基準
app_schema:
  base_dir: "./.agent/climpt/schema/app" # スキーマ機能の基準
```
...
2. アプリケーション設定ファイルは`{ベースディレクトリ}/.agent/climpt/config/`から読み込み
3. ユーザー設定ファイルは`{working_dir}/.agent/climpt/config/`から読み込み
...
| **配置規則** | `.agent/climpt/config/` 配下への統一配置 |
