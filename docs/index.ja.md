...
3. **アプリケーション設定ファイルの読み込み**: `{ベースディレクトリ}/.agent/clipmt/config/app.yml` または `{ベースディレクトリ}/.agent/clipmt/config/{プロファイルプレフィックス}-app.yml` を必須ファイルとして読み込み
4. **working_dirの特定**: アプリケーション設定ファイルから `working_dir` の値を取得してユーザー設定ファイルの基準パスを決定
5. **ユーザー設定ファイルの読み込み**: `{working_dir}/.agent/clipmt/config/user.yml` または `{working_dir}/.agent/clipmt/config/{プロファイルプレフィックス}-user.yml` をオプションファイルとして読み込み
...
let config = new BreakdownConfig();
// → {ベースディレクトリ}/.agent/clipmt/config/app.yml と {working_dir}/.agent/clipmt/config/user.yml を読み込み

// 名前付きプロファイルの読み込み
let devConfig = new BreakdownConfig("development");
// → {ベースディレクトリ}/.agent/clipmt/config/development-app.yml と {working_dir}/.agent/clipmt/config/development-user.yml を読み込み

let prodConfig = new BreakdownConfig("production");
// → {ベースディレクトリ}/.agent/clipmt/config/production-app.yml と {working_dir}/.agent/clipmt/config/production-user.yml を読み込み
```
...
| -------------------- | --------------------- | ---------------------------------------------- |
| 未指定（デフォルトプロファイル） | `{ベースディレクトリ}/.agent/clipmt/config/app.yml`             | `{working_dir}/.agent/clipmt/config/user.yml`             |
| "development"        | `{ベースディレクトリ}/.agent/clipmt/config/development-app.yml` | `{working_dir}/.agent/clipmt/config/development-user.yml` |
| "production"         | `{ベースディレクトリ}/.agent/clipmt/config/production-app.yml`  | `{working_dir}/.agent/clipmt/config/production-user.yml`  |
| "{任意の名前}"           | `{ベースディレクトリ}/.agent/clipmt/config/{任意の名前}-app.yml`    | `{working_dir}/.agent/clipmt/config/{任意の名前}-user.yml`    |
...
- パス `{ベースディレクトリ}/.agent/clipmt/config/app.yml` から読み込み
- 必須。省略できない。
...
- パス `{working_dir}/.agent/clipmt/config/user.yml` から読み込み
...
- パス `{ベースディレクトリ}/.agent/clipmt/config/{プロファイルプレフィックス}-app.yml` から読み込み
- 必須。省略できない。
- 設定ファイルが存在しない場合はエラーで終了する。
- `working_dir` 設定がユーザー設定ファイルの起点ディレクトリとなる。
...
- パス `{working_dir}/.agent/clipmt/config/{プロファイルプレフィックス}-user.yml` から読み込み
- 存在がなくても正常処理とする。（warning出力を行うのみ）
- 設定値を読み込み、同一キーのアプリケーション設定値を上書きする。
  - ユーザー設定ファイルは必要な設定のみ記述できる。全ての項目が任意である。
...
- **未指定時**: デフォルトプロファイル（{ベースディレクトリ}/.agent/clipmt/config/app.yml, {working_dir}/.agent/clipmt/config/user.yml）を使用
- **指定時**: 指定されたプロファイルプレフィックスを持つ名前付きプロファイル（{ベースディレクトリ}/.agent/clipmt/config/{プロファイルプレフィックス}-app.yml, {working_dir}/.agent/clipmt/config/{プロファイルプレフィックス}-user.yml）を使用
