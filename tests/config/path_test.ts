...
Deno.test({
  ...
  async fn() {
    const tempDir = await setupTestConfigs(validAppConfig, validUserConfig, TEST_WORKING_DIR);
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      const userConfigPath = join(tempDir, TEST_WORKING_DIR, "config", "user.json");
