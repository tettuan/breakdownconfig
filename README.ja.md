...
- **`baseDir`** (オプション): 設定ファイルのベースディレクトリ
  - デフォルトは現在の作業ディレクトリ (`""`)
  - 設定ファイルは `{baseDir}/.agent/climpt/config/` から読み込み
...
// 読み込み: /path/to/project/.agent/climpt/config/app.yml

// 環境固有 + カスタムベースディレクトリ
const envResult = await BreakdownConfig.loadConfig("staging", "/path/to/project");
// 読み込み: /path/to/project/.agent/climpt/config/staging-app.yml
...
// → カレントディレクトリの ./.agent/climpt/config/ から読み込み
```

**読み込まれるファイル**:

- アプリケーション設定: `./.agent/climpt/config/app.yml` （必須）
- ユーザー設定: `./.agent/climpt/config/user.yml` （オプション）
...
// → /path/to/project/.agent/climpt/config/ から読み込み
```
...
// → ./.agent/climpt/config/production-app.yml と production-user.yml から読み込み
```
...
**デフォルト設定**: `./.agent/climpt/config/app.yml`
**環境固有設定**: `./.agent/climpt/config/{prefix}-app.yml`
...
**デフォルト設定**: `./.agent/climpt/config/user.yml`
**環境固有設定**: `./.agent/climpt/config/{prefix}-user.yml`
...
- アプリケーション設定とユーザー設定は同じディレクトリ（`./.agent/climpt/config/`）に配置されます
...
すべてのファイルは `./.agent/climpt/config/` ディレクトリに配置されます。
...
```yaml
working_dir: "./.agent/climpt" # 継承
app_prompt:
  base_dir: "./.agent/climpt/prompts/feature-x" # 上書き
